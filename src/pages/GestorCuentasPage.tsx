...
import { Upload, Download, BarChart3, Coins } from 'lucide-react';
...
export const GestorCuentasPage: React.FC<GestorCuentasPageProps> = ({ 
  ...
  const {
    ...
    registrarPagoParcialTotal: registrarPagoParcialTotalHook,
  ...
  const totalDeudaClienteSeleccionado = useMemo(() => {
    if (!clienteSeleccionado) return 0;
    return stableCalculateDeuda(clienteSeleccionado.id);
  }, [clienteSeleccionado, stableCalculateDeuda]);
  ...
  const handleCerrarPagoParcialTotalModal = () => {
    setShowPagoParcialTotalModal(false);
    setPagoParcialTotalAmount('');
  };
  ...
  return (
    <>
            ...
            <>
              <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 pb-4 border-b border-gray-200">
                ...
                <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
                  ...
                  <button
                    onClick={handleAbrirPagoParcialTotalModal}
                    className="bg-blue-500 text-white px-3 py-2 rounded-md hover:bg-blue-600 transition-colors text-sm w-full sm:w-auto flex items-center gap-2"
                    disabled={totalDeudaClienteSeleccionado === 0}
                  >
                    <Coins size={16} /> Pago Parcial Total
                  </button>
                  <button
      ...
      <ConfirmationModal
        isOpen={showPagoParcialTotalModal}
        title="Registrar Pago Parcial Total"
        onConfirm={handleConfirmarPagoParcialTotal}
        onCancel={handleCerrarPagoParcialTotalModal}
